...
同モデル、1 ケースあたり 5 回、キャッシュ無効で測定しました。8 ケース全体で素通し
**8.75** に対し PhiGate 経由 **8.88**（+0.12）です。

**+0.12 は「PhiGate が回答を良くする」という意味ではありません。** ケースごとの
最大ばらつきが ±2.14 ある以上、0.1 点の差は審査モデルのノイズです。正しい読み方は
**圧縮と匿名化による品質差は測定できなかった**であり、この表が支持するのはその主張
だけです。

`disk-full-remediation` には経緯があります。同じベンチマークの以前の実行では、この
ケースは素通し 9.00 に対し **1.60** でした。4 段階の復旧手順のうち 1 つに含まれた
`find ... -delete` を理由に、エグレスガードが**回答全体**を差し止めたためです
（`du` による調査、`lsof` による削除済みオープンファイルの確認、ログローテーション
の助言もろとも消えました）。これは実在の欠陥であり、議論ではなくこのベンチマークが
見つけ、すでに修正済みです。現在は該当箇所のみを切り取って残りを返します。上の表は
修正後のビルドで同じケースを測り直した数値です。欠陥を露呈させた −7.40 は
[変更履歴](CHANGELOG.md)に残してあります。あとから黙って良くなる数字こそ、読者が
疑うべきものだからです。

**訂正を記録しておきます。** 以前の実行では `tls-cert-expiry` が −1.00 となり、
この README はその理由を「`internal_hostname` が `api.internal.corp` を `<V1>` に
マスクし、そのホスト名こそが自社 CA を示す手がかりだった」と説明していました。
マスク自体は実際に起きており、知っておく価値があります。しかし**そこに品質コストを
帰属させた点は支持できません** — 同じケースが今回は **+1.20** であり、いずれも
±1.72 のばらつきの内側です。1 回の標本から物語を組み立てたもので、下に挙げた 2 つ目
の但し書きが警告しているのはまさにこれです。機構の説明は残りますが、値札は取り下げ
ます。

同じ実行での削減率は **64.2%**（前回と同一 — 圧縮とルーティングは決定的なので、
審査による列だけが動きます）ですが、その大半は圧縮ではなく**ルーティング**に
...
<details>
<summary><b>ID プロバイダ連携（OIDC）</b> — 自社 IdP の発行するトークンで認証する</summary>

静的 API キーは、PhiGate が発行し・保管し・誰かが手で更新する資格情報です。
大企業にはすでにその 3 つを担う ID 基盤があり、セキュリティ審査では必ず
「なぜそれを使わないのか」を問われます。

| 変数 | 用途 |
|---|---|
| `PHIGATE_OIDC_ISSUER` | トークンが持つべき `iss`。**未設定ならこの機能全体が無効** |
| `PHIGATE_OIDC_AUDIENCE` | `aud` に含まれていなければならない値 |
| `PHIGATE_OIDC_JWKS_URL` | 署名鍵の公開先。未設定なら issuer から導出 |
| `PHIGATE_OIDC_TENANT_CLAIM` | 呼び出し元のグループ／ロールを持つクレーム（例 `groups`） |
| `PHIGATE_OIDC_TENANT_MAP` | `グループ:テナント,...` — `PHIGATE_API_KEYS` と同じ書式 |

**静的キーは併用できます。** 両方が同時に有効なので、移行は一斉切替ではなく
クライアント単位で進められます。資格情報はまず設定済みキーと照合され、
どれとも一致せず JWT の形をしている場合にのみトークン検証へ進みます。

**検証する項目**：プロバイダの JWKS から取得した鍵による**署名**（未知の鍵 ID は
最大 1 分に 1 回だけ再取得するため、鍵のローテーションには追従しつつ、不正な
トークンの連投で IdP を叩き続けることはありません）。**アルゴリズム**（`none`
と HMAC 系は拒否 — RSA 公開鍵を持つ検証器が HS256 を受け入れると、公開鍵を共有
秘密として署名されたトークンを通してしまいます）。**issuer** と **audience** の
完全一致。**有効期限**と **nbf**（60 秒のずれを許容、`exp` の無いトークンは拒否）。
そして**テナントクレーム**——どこにも対応しないグループは、既定テナントとして
通すのではなく拒否します。設定されていない対応付けを推測することは、誰も与えて
いない権限を与えることだからです。

拒否されたトークンには理由を返します（`audience`、`expired` など）。理由の見えない
連携は 1 日を溶かしますが、トークンの保持者がそこから新たに得る情報はありません。
一方、静的キーの失敗は一般的なメッセージのままです——具体的に答えることは、キーを
推測するためのオラクルになるためです。

検証は JOSE ライブラリではなく `crypto/rsa` と `crypto/ecdsa` で直接実装して
います。`internal/metrics` が Prometheus 形式を直接扱うのと同じ理由で、
`make ce-purity` はコミュニティ版が tree-sitter 以外に依存しないことを検査して
おり、一晩で読み切れる依存関係のほうが数百行の節約より価値があるためです。
